...
Figure \ref{fig:figs} shows the distribution of genres in the training set. The drama genre is by far the most represented, followed by comedy, thriller, and romance. The IMAX, film-noir, and Western are the least represented genres.

```{r figs, label = "histogram_genres", echo = TRUE, results = "show", fig.cap = "\\label{fig:figs} Histogram of genres"}
...
Figure \ref{fig:figs} shows the distribution of ratings in the training set. The most frequent rating (mode) is `r edx_train |> group_by(rating) |> summarize(n = n(), per = label_percent(accuracy = .1)(n / nrow(edx_train))) |> filter(n == max(n)) |> pull(rating)` (`r edx_train |> group_by(rating) |> summarize(n = n(), per = label_percent(accuracy = .1)(n / nrow(edx_train))) |> filter(n == max(n)) |> pull(per)`). The least frequent rating is `r edx_train |> group_by(rating) |> summarize(n = n(), per = label_percent(accuracy = .1)(n / nrow(edx_train))) |> filter(n == min(n)) |> pull(rating)` (`r edx_train |> group_by(rating) |> summarize(n = n(), per = label_percent(accuracy = .1)(n / nrow(edx_train))) |> filter(n == min(n)) |> pull(per)`). The mean of all the ratings is `r round(x = mean(edx_train$rating), digits = 2)`. There is no zero rating.

```{r figs, label = "histogram_ratings", echo = TRUE, results = "show", fig.cap = "\\label{fig:figs} Histogram of ratings"}
...
Figure \ref{fig:figs} shows the number of movies that have received a given number of ratings. It confirms that some movies receive few ratings (left side of the chart) while few movies receive many ratings (right tail).

```{r figs, label = "scatterplot_ratings", echo = TRUE, results = "show", fig.cap = "\\label{fig:figs} Scatterplot of number of ratings per movie"}
...
These observations are confirmed by plotting the average rating for every count of movie ratings. Figure \ref{fig:figs} shows that movies with few ratings (those near the vertical axis) have a much wider rating (vertical) spread than those with many ratings, which tend to stabilize around the mode of `r edx_train |> group_by(rating) |> summarize(n = n(), per = label_percent(accuracy = .1)(n / nrow(edx_train))) |> filter(n == max(n)) |> pull(rating)`.

```{r figs, label = "scatterplot_mean_rating_per_rating_count", echo = TRUE, results = "show", fig.cap = "\\label{fig:figs} Scatterplot of mean rating per rating count"}
...
Lastly, a similar discrepancy in ratings was observed across users. Figure \ref{fig:figs} plots the mean rating assigned by every user (on the vertical axis) against the number of times those users have given ratings. Many users who contributed very few ratings display a wider (vertical) spread in their ratings, relative to frequent reviewers who tend to assign that are, on average, close to the overall average of `r round(x = mean(edx_train$rating), digits = 2)`.

```{r figs, label = "scatterplot_mean_rating_per_user", echo = TRUE, results = "show", fig.cap = "\\label{fig:figs} Scatterplot of mean rating per user"}
